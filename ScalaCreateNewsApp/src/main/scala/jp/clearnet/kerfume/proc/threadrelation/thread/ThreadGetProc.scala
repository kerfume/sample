...
object ThreadGetProc extends DaoTrait{
  ...
  //スレッド行判定パターン
  private val threadPattern:Pattern = Pattern.compile("<a.*\\(\\d+\\)</a>")
  //スレッドタイトル判定パターン
  private val threadTitlePattern:Pattern = Pattern.compile("\\d+: .*\\(\\d+\\)</a>$")
  //レスポンス数判定パターン
  private val responsNumPattern:Pattern = Pattern.compile("\\(\\d+\\)$")
  //スレッドID（URL）判定パターン
  private val threadIdPattern:Pattern = Pattern.compile("^<a href=\"\\d+/l50\">")
          ...
          session.withTransaction {
            ...
            while ( {
              ...
              if (matcher(buffLine)) {
                ...
                //スレッドDtoの取得
                if ( {
                  threadDto = getThreadInf(buffLine)
